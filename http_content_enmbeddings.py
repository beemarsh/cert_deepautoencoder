...
http_df = pd.read_csv("http.csv")

# Clean the email content and apply the embedding
http_df['content_cleaned'] = http_df['content'].apply(remove_unnecessary_words).apply(remove_punctuation)
http_df['content_vector'] = http_df['content_cleaned'].apply(general_vector_rep)

# Convert the embedded vectors into separate columns (c0, c1, ..., c299)
embedded_df = pd.DataFrame(http_df['content_vector'].tolist(), columns=[f'c{i}' for i in range(300)])

# Concatenate the original email data (minus 'Content') with the new embedded columns
http_df = pd.concat([http_df.drop(['content', 'content_cleaned'], axis=1), embedded_df], axis=1)

# Save to a new CSV file
http_df.to_csv("http_with_embeddings.csv", index=False)
